...
function _addConn(p0, p1, active) {
  ...
  // THREE.LineBasicMaterial.linewidth is ignored by WebGL on most platforms,
  // so we use TubeGeometry + MeshBasicMaterial for visible, thick connections.
  function makeTube(sign, color, opacity) {
    const curve = new THREE.CatmullRomCurve3(bezPts(sign));
    const geo   = new THREE.TubeGeometry(curve, 24, 0.18, 8, false);
    const mat   = new THREE.MeshBasicMaterial({ color, transparent: true, opacity, depthTest: false, depthWrite: false });
    const mesh  = new THREE.Mesh(geo, mat);
    mesh.renderOrder = 9;
    return { mesh, geo, mat };
  }
  const t1 = makeTube( 1, active ? 0x22c97d : 0x2aa8bf, active ? 0.80 : 0.65);
  const t2 = makeTube(-1, active ? 0xe8823a : 0x2aa8bf, active ? 0.72 : 0.55);
