...
class CheckpointManager:
    ...
    def __init__(
        ...
        self.save_optimizer = (
            save_optimizer_state if save_optimizer_state is not None else save_optimizer
        )
        self.rank = rank
        ...
        logger.info(
            f"Checkpoint manager initialized: dir={self.checkpoint_dir}, max_keep={max_checkpoints}"
        )
    ...
    def save_checkpoint(
        ...
        try:
            ...
            logger.info(
                f"Checkpoint saved: {checkpoint_path} (step={step}, epoch={epoch}, loss={loss})"
            )
    ...
    def load_checkpoint(
        ...
        try:
            ...
            logger.info(
                f"Checkpoint loaded: step={metadata['step']}, epoch={metadata['epoch']}, loss={metadata['loss']}"
            )
    ...
    def _cleanup_old_checkpoints(self):
        ...
        for checkpoint_path in checkpoints[:num_to_delete]:
            try:
                ...
                logger.warning(
                    f"Failed to remove old checkpoint {checkpoint_path}: {e}"
                )
