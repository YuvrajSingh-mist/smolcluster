...
def linear_quantize(tensor, dtype=torch.int8):
    ...
    if r_max - r_min <= 1e-5:
        ...
        zero_point = torch.round(torch.tensor(q_min - (r_min / scale), device=tensor.device))
...
def dequantize_model_weights(quantized_dict: Dict[str, torch.Tensor], device: Union[str, torch.device] = 'cpu'):
    """
